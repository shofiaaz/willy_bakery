...
@section('content')
<h1 class="text-2xl font-bold text-bread-800 mb-6">Edit Resep: {{ $product->product_name }}</h1>

<form action="{{ route('produksi.recipe.update', $product->product_id) }}" method="POST"
    class="bg-white p-6 rounded-2xl card-shadow">
    @csrf
    @method('PUT')
        ...
        <div class="flex gap-3 mb-3">
            <select name="material_id[]" class="border p-3 rounded-xl w-1/2">
                @foreach ($materials as $m)
                    <option value="{{ $m->material_id }}" {{ $m->material_id == $recipe->material_id ? 'selected' : '' }}>
                        {{ $m->material_name }}
                    </option>
                @endforeach
            </select>

            <input type="number" name="quantity_needed[]" value="{{ $recipe->quantity_needed }}"
